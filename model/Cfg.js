...
// 插件根目录：本文件位于 <插件根>/model/Cfg.js，向上两级即为插件根目录
const _pluginRoot = path.dirname(path.dirname(fileURLToPath(import.meta.url)));

class Cfg {
  constructor(pluginName) {
    // 默认使用插件自身所在目录，这样插件目录改名后依然能读到配置
    // 传入 pluginName 时则按 Yunzai 的 plugins/<name> 定位
    const pluginPath = pluginName
      ? path.join(process.cwd(), 'plugins', pluginName)
      : _pluginRoot;
    this.pluginPath = pluginPath.replace(/\\/g, '/');
