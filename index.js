...
function getVersion() {
  try {
    ...
    logger.error('[chuo插件] 读取版本号失败:', err)
    return '0.0.0'
  }
}
...
function initConfig() {
  ...
  try {
    ...
    if (!fs.existsSync(configDir)) {
      ...
      logger.mark('[chuo插件] 创建配置目录')
    }

    // 检查默认配置目录是否存在
    if (!fs.existsSync(defaultConfigDir)) {
      logger.error('[chuo插件] 默认配置目录不存在:', defaultConfigDir)
      return result
    }
    ...
    if (defaultConfigs.length === 0) {
      logger.warn('[chuo插件] 默认配置目录中没有找到配置文件')
      return result
    }
    ...
    defaultConfigs.forEach(file => {
      ...
      if (!fs.existsSync(target)) {
        ...
        result.new++
        logger.debug(`[chuo插件] 已复制默认配置: ${file}`)
      } else {
        result.existing++
      }
    })
    ...
    if (result.new > 0) {
      logger.mark(`[chuo插件] 已初始化 ${result.new} 个新配置文件`)
    }
    if (result.existing > 0) {
      logger.mark(`[chuo插件] 跳过 ${result.existing} 个已存在的配置文件`)
    }

    return result
  } catch (err) {
    logger.error('[chuo插件] 配置初始化失败:', err)
    return result
  }
}
...
async function loadApps() {
  ...
  try {
    ...
    if (!fs.existsSync(appsPath)) {
      logger.error('[chuo插件] 模块目录不存在:', appsPath)
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (exportKey) {
          ...
          result.success++
          logger.debug(`[chuo插件] 成功加载模块: ${name}`)
        }
      } catch (err) {
        result.failed++
        logger.warn(`[chuo插件] 加载模块 ${name} 失败:`, err)
      }
    }

    const duration = Date.now() - startTime
    // logger.mark(`[chuo插件] 加载完成，成功 ${result.success} 个，失败 ${result.failed} 个，耗时 ${logger.green(duration + 'ms')}`)
    
    return { ...result, duration }
  } catch (err) {
    logger.error('[chuo插件] 加载过程出错:', err)
    throw err
  }
}
